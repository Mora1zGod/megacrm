...
const STAGE_COLORS = ['#0E9AA0', '#8FE3DC', '#10B981', '#FBBF24', '#F87171', '#A78BFA', '#94A3B8'];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <div
        ...
        className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-[rgba(14,154,160,0.25)] bg-[#062720] shadow-[0_0_40px_rgba(14,154,160,0.15)]"
      >
        <div className="flex items-center justify-between border-b border-[rgba(14,154,160,0.12)] px-5 py-4">
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-2">
              {pipelines.map((p) => (
                <div
                  key={p.id}
                  className={`rounded-lg border px-3 py-2 ${p.id === selectedId ? 'border-[var(--accent-primary)] bg-[rgba(14,154,160,0.06)]' : 'border-[rgba(14,154,160,0.15)]'}`}
                >
                  ...
                  <input
                    ...
                    className="mt-1 w-full rounded border border-[rgba(14,154,160,0.15)] bg-transparent px-2 py-1 text-xs text-[var(--color-text-secondary)] outline-none focus:border-[var(--accent-primary)]"
                  />
                </div>
              ))}
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={handleCreateFunil} disabled={busy || !newFunil.trim()} className="rounded-lg bg-gradient-to-br from-[#1E3A8A] to-[#0E9AA0] px-3 py-2 text-sm font-semibold text-white disabled:opacity-60">
          ...
          <section className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="rounded-lg bg-gradient-to-br from-[#1E3A8A] to-[#0E9AA0] px-3 py-2 text-sm font-semibold text-white disabled:opacity-60"
              >
                <Plus className="h-4 w-4" />
              </button>
            </div>
        ...
        <div className="flex items-center gap-1">
          {STAGE_COLORS.map((c) => (
            <button
              ...
              className={`h-3.5 w-3.5 rounded-full ring-offset-1 ring-offset-[#062720] ${stage.color === c ? 'ring-2 ring-white' : ''}`}
              style={{ background: c }}
              aria-label={`Cor ${c}`}
            />
          ))}
