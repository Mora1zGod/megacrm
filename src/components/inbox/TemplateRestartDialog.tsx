...
export function TemplateRestartDialog({ open, onClose, conversationId, onSent }: Props) {
  ...
  const send = async () => {
    ...
    const faltando = vars.filter((idx) => !(values[idx] ?? '').trim());
    if (faltando.length > 0) {
      toast.error(
        vars.length === 1
          ? 'Preencha o valor da variável antes de enviar.'
          : `Preencha o valor de ${faltando.map((i) => `{{${i}}}`).join(', ')} antes de enviar.`,
      );
      return;
    }
    ...
    setSending(false);
    if (error || !data?.ok) {
      const detalhe = await extractFunctionErrorMessage(error, data?.error);
      toast.error('Falha ao enviar template', { description: detalhe });
      return;
    }
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Reiniciar com template" widthClass="max-w-lg">
      <div className="space-y-4">
        ...
        {selected && (
          <div className="rounded-lg border border-[rgba(14,154,160,0.12)] bg-white/[0.02] p-3 text-sm text-[var(--color-text-secondary)] whitespace-pre-wrap">
            {selected.body}
          </div>
        )}
