...
export function MessageInput({ conversationId, disabled, withinWindow = true, onSendText }: MessageInputProps) {
  ...
  // "Melhorar": reescreve o rascunho com o tom da marca. Guarda o texto
  // original para permitir desfazer — o operador precisa poder voltar se a
  // reescrita não ficou boa, sem ter que redigitar tudo.
    ...
    try {
      ...
      if (error || !data?.ok) {
        toast.error('Falha ao enviar áudio', {
          description: await extractFunctionErrorMessage(error, data?.error),
        });
      }
    } finally {
      setSending(false);
  ...
  // Reescreve o rascunho aplicando o tom da marca (Edge Function
  // improve-message). NÃO envia nada — só substitui o texto na caixa para o
  // operador revisar. O original fica guardado para desfazer.
  const improveDraft = async () => {
    ...
    try {
      ...
      setPreImprove(draft);
      setContent(data.improved as string);
    } catch (err) {
      toast.error('Não consegui melhorar a mensagem.', {
