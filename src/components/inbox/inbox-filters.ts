...
export function matchesFilters(
  ...
  if (f.queueId === 'sem_fila') {
    if (c.queue_id) return false;
  } else if (f.queueId !== 'any') {
    if (c.queue_id !== f.queueId) return false;
  }
...
export function writeFiltersToParams(
  ...
  setOrDel('ffi', f.queueId, f.queueId !== 'any');
  return next;
}
