      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          {DAY_ORDER.map(({ key, label }) => (
            <div
              key={key}
              className="grid grid-cols-[auto_100px_1fr_1fr] items-center gap-3 p-3 rounded-lg border border-[rgba(14,154,160,0.1)] bg-white/[0.02]"
            >
              <input
                type="checkbox"
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            rows={3}
            disabled={saving}
            className="w-full rounded-lg border border-[rgba(14,154,160,0.2)] bg-white/[0.03] px-4 py-3 text-sm text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--accent-primary)] focus:bg-white/[0.06]"
          />
        </div>
