...
import { useAttentionItems, useTodayVisits } from '@/hooks/useAttentionItems';
...
export default function DashboardPage() {
  ...
  const { role, displayName } = useAppUser();
  const { user } = useAuth();
  const { costs, save: saveCosts } = useSalesCosts();
  const [refreshing, setRefreshing] = useState(false);
  const navigate = useNavigate();

  // Dashboard 2.0 — dados reais pra seção de topo.
  const dashMetrics = useDashboardMetrics();
  const { items: attentionItems, loading: attentionLoading } = useAttentionItems();
  const { visits: todayVisits, loading: visitsLoading } = useTodayVisits();
  const { stages: pipelineStages, deals: pipelineDeals } = usePipeline();

  const miniStages = useMemo(
    () => pipelineStages
      .filter((s) => !s.is_won && !s.is_lost)
      .map((s) => {
        const dealsHere = pipelineDeals.filter((d) => d.stage_id === s.id && d.status === 'open');
        return { id: s.id, name: s.name, count: dealsHere.length, value: dealsHere.reduce((sum, d) => sum + (Number(d.value) || 0), 0) };
      }),
    [pipelineStages, pipelineDeals],
  );

  // Taxa de conversão real: ganhos / (ganhos + perdidos) no período. Sem
  // nenhum dos dois, não dá pra calcular — mostra "—" em vez de inventar 0%.
  const conversionRate = metrics.won.count + metrics.lost.count > 0
    ? (metrics.won.count / (metrics.won.count + metrics.lost.count)) * 100
    : null;

  const firstName = (displayName?.trim().split(/\s+/)[0]) || (user?.email ?? '').split('@')[0];
  const greeting = (() => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* Dashboard 2.0 — saudação */}
      <div>
        <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">{greeting}, {firstName || 'tudo bem'}!</h1>
      ...
      {/* Evolução + Atendimentos por canal */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <EvolutionWidget data={dashMetrics.volumeByDay} />
        <OriginDonutWidget
          title="Atendimentos por canal"
          data={metrics.origin.messageChannel}
          labelMap={MESSAGE_CHANNEL_LABEL}
          emptyText="Nenhuma conversa no período."
        />
      </div>

      {/* Precisam de atenção + Hoje na agenda */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <AttentionListWidget items={attentionItems} loading={attentionLoading} onNavigate={navigate} />
        <TodayAgendaWidget visits={todayVisits} loading={visitsLoading} />
      </div>

      {/* Pipeline comercial (resumo — funil completo é outra tela) */}
      <MiniPipelineWidget stages={miniStages} onOpen={() => navigate('/funil')} />

      <div className="flex items-start justify-between flex-wrap gap-4 pt-2 border-t border-[var(--color-border-card)]">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-label">Detalhado</div>
            <h2 className="text-xl font-bold text-display">Métricas de vendas</h2>
            <p className="text-sm text-[var(--color-text-secondary)]">Widgets configuráveis — personalize abaixo</p>
          </div>
        </div>
        ...
        {periodKey === 'custom' && (
          <div className="flex items-center gap-2">
            <input
              ...
              className="rounded-lg border border-[rgba(14,154,160,0.2)] bg-white/[0.03] px-2 py-1 text-xs text-[var(--color-text-primary)]"
            />
      ...
      {customizeOpen && (
        <div className="glass-card p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
            {WIDGETS.map((w) => (
              <label key={w.key} className="flex items-center gap-2 rounded-lg border border-[rgba(14,154,160,0.12)] bg-white/[0.02] px-3 py-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={show(w.key)}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          {show('uso_ia') && (
            <AiUsageWidget
              messagesCount={metrics.ai.messagesCount}
              conversationsCount={metrics.ai.conversationsCount}
              costUsd={metrics.ai.costUsd}
            />
          )}
          {show('origem_trafego') && (
...
function SalesCostsForm({
  ...
  const inputCls =
    'w-full rounded-lg border border-[rgba(14,154,160,0.2)] bg-white/[0.03] px-2 py-1.5 text-sm text-[var(--color-text-primary)] outline-none focus:border-[var(--accent-primary)] disabled:opacity-50';
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <select
              ...
              disabled={!canEdit}
              className="rounded-lg border border-[rgba(14,154,160,0.2)] bg-white/[0.03] px-2 py-1.5 text-sm text-[var(--color-text-primary)] outline-none focus:border-[var(--accent-primary)] disabled:opacity-50 [&>option]:bg-[#062720]"
            >
