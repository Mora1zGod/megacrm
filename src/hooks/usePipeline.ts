  ...
  const loadBoard = useCallback(async (pipelineId: string | null) => {
    ...
    if (contactIds.length > 0) {
      const { data: convs } = await supabase
        .from('conversations')
        .select('id, contact_id, channel, last_message_at, provider')
        .in('contact_id', contactIds);
      const cmap: Record<string, ContactConvInfo[]> = {};
      for (const c of (convs ?? []) as Array<{ id: string; contact_id: string; channel: string | null; last_message_at: string | null; provider: string | null }>) {
        (cmap[c.contact_id] ??= []).push({ id: c.id, channel: c.channel, last_message_at: c.last_message_at, provider: c.provider });
      }
