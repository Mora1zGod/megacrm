...
export function useKnowledgeBase(): UseKnowledgeBaseResult {
  ...
  const addAndProcess: UseKnowledgeBaseResult['addAndProcess'] = async ({ name, source_type, content }) => {
    ...
    if (fnErr || !data?.ok) {
      ...
      // itself should have flipped status=error.
      let message = data?.error ?? (await extractFunctionErrorMessage(fnErr, null));
      message = friendlyOpenAiError(message || 'Falha ao processar');
      setError(message);
      await reload();
