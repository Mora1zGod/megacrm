...
export interface SalesMetrics {
  ...
  origin: {
    ...
    // Canal de MENSAGEM (whatsapp/instagram/uazapi) — não depende de UTM,
    // vem direto de `conversations`. Populado desde a 1ª conversa.
    messageChannel: NameCount[];
  };
  // Uso da IA no período: quantas mensagens ela mandou, em quantas
  // conversas diferentes, e o custo estimado (soma de whatsapp_hub.ai_usage_log
  // — embedding + visão + chat + auto-move de estágio).
  ai: { messagesCount: number; conversationsCount: number; costUsd: number };
...
const EMPTY_METRICS: SalesMetrics = {
  ...
  origin: { hasData: false, traffic: [], channel: [], campaign: [], ads: [], organicPosts: [], messageChannel: [] },
  ai: { messagesCount: 0, conversationsCount: 0, costUsd: 0 },
...
function compute(range: PeriodRange, deals: DealRow[], stageProb: Map<string, number>, openDeals: { value: number | null; stage_id: string | null }[], conversations: ConversationChannelRow[], aiMessages: { conversation_id: string | null }[], aiCostRows: { estimated_cost_usd: number | string }[]): SalesMetrics {
  ...
  // Canal de mensagem: whatsapp (Meta oficial) × whatsapp (UAZAPI, número
  // próprio) × instagram. Sempre populado — não depende de UTM.
  const messageChannel = groupCount(
    conversations.map((c) => ({
      key: c.channel === 'instagram' ? 'instagram' : c.provider === 'uazapi' ? 'whatsapp_uazapi' : 'whatsapp_meta',
      value: 1,
    })),
  );
  ...
  // Uso da IA: total de mensagens dela no período + em quantas conversas
  // diferentes ela apareceu (uma conversa pode ter várias respostas) + custo
  // estimado somado (embedding + visão + chat + auto-move, todas as chamadas).
  const aiMessagesCount = aiMessages.length;
  const aiConversationsCount = new Set(
    aiMessages.map((m) => m.conversation_id).filter((id): id is string => Boolean(id)),
  ).size;
  const aiCostUsd = aiCostRows.reduce((sum, r) => sum + Number(r.estimated_cost_usd ?? 0), 0);
  ...
  return {
    ...
    firstResponse,
    closeTime,
    origin: { hasData, traffic, channel, campaign, ads, organicPosts, messageChannel },
    ai: { messagesCount: aiMessagesCount, conversationsCount: aiConversationsCount, costUsd: aiCostUsd },
...
export function useSalesDashboard(range: PeriodRange) {
  ...
  const reload = useCallback(async () => {
    ...
    const [createdRes, wonRes, lostRes, openRes, stagesRes, convsRes, aiMsgRes, aiCostRes] = await Promise.all([
      ...
      supabase.from('conversations').select('channel, provider, created_at').gte('created_at', fromISO).lte('created_at', toISO),
      supabase.from('messages').select('conversation_id').eq('sender_type', 'ai').gte('created_at', fromISO).lte('created_at', toISO),
      supabase.from('ai_usage_log').select('estimated_cost_usd').gte('created_at', fromISO).lte('created_at', toISO),
    ]);

    const firstErr = createdRes.error || wonRes.error || lostRes.error || openRes.error || stagesRes.error || convsRes.error || aiMsgRes.error || aiCostRes.error;
    if (firstErr) {
    ...
    const conversations = (convsRes.data ?? []) as ConversationChannelRow[];
    const aiMessages = (aiMsgRes.data ?? []) as { conversation_id: string | null }[];
    const aiCostRows = (aiCostRes.data ?? []) as { estimated_cost_usd: number | string }[];

    setMetrics(compute(range, deals, stageProb, openDeals, conversations, aiMessages, aiCostRows));
    setLoading(false);
  }, [range.from, range.to]);
